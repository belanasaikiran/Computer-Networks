...
def searchQuery(query, request, missing_query_locations, query_combos):
    print("searchn query called with ", query)
        ...
        for location in range(len(missing_query_locations)):
            for alphabet in string.ascii_lowercase: # add all letters for search
                ...
                if location == 0:
                    ...
                    for query_index in range(len(query_combos)):
                        searchable = list(query_combos[query_index]) #convert request to list
                        searchable[missing_query_locations[location]] = alphabet
                        searchable = ''.join(searchable)
                        # print("Request Searchable: ", searchable)
                        query_combos[query_index] = searchable
    return query_combos, missing_query_locations
...
def handleRequest(request): # function for handling the request
    ...
    print("len(missing_query_locations): ", len(missing_query_locations))
